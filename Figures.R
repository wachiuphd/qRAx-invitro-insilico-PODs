...
res.df$name[res.df$name=="PPRTV"] <- "qRAx"
res.df$name[res.df$name=="Aurisano"] <- "ToxValDB"
res.df$name[res.df$name=="ToxCast"] <- "ToxCast"
res.df$name[res.df$name=="CTV"] <- "QSAR#1"
res.df$name[res.df$name=="Kvasnicka"] <- "QSAR#2"
res.df$name[res.df$name=="von Borries"] <- "QSAR#3"
...
res.success$name[res.success$name=="PPRTV"] <- "qRAx"
res.success$name[res.success$name=="Aurisano"] <- "ToxValDB"
res.success$name[res.success$name=="ToxCast"] <- "ToxCast"
res.success$name[res.success$name=="CTV"] <- "QSAR#1"
res.success$name[res.success$name=="Kvasnicka"] <- "QSAR#2"
res.success$name[res.success$name=="von.Borries"] <- "QSAR#3"
...
psuccess <-
  ggplot(subset(res.success,!is.na(name)))+
    ...
    ggtitle("Number (and %) of Derived PODs")+
    xlab("")+ylab("")+geom_hline(yintercept = 41,linetype="dashed")
print(psuccess)
ggsave(file.path(resultsfolder,"Fig3A-success.pdf"),psuccess,height=4,width=6,scale=0.65)
ggsave(file.path(resultsfolder,"Fig3A-success.svg"),psuccess,height=4,width=6,scale=0.65)

chem.names <- as.character(res.df$Chemical)
names(chem.names)<-res.df$CASRN
chem_labs <- unique(chem.names)
CASRN <- unique(names(chem.names))
names(chem_labs) <- chem_labs
chem_labs["1-Phenyl-1-(2,4-dimethylphenyl)-ethane (PXE)"]<-"PXE"
chem_labs["1-Phenyl-1-(4-methylphenyl)-ethane (PTE)"]<-"PTE"
chem_labs["N,N,N',N'' Tetramethylphosphoramide (TMPA)"]<-"TMPA"
chem_labs["Pentamethylphosphoramide (PMPA)"]<-"PMPA"
chem_labs["Picramic Acid (2-Amino-4,6-dinitrophenol)" ]<-"Picramic Acid"
chem_labs["Picric Acid (2,4,6-Trinitrophenol)" ]<-"Picric Acid"
names(chem_labs) <- CASRN
...
ppods <-
  ...
  theme(legend.position = "none",
    ...
    scale_color_viridis_d(direction = -1, end=0.9)+ylab("")+
    facet_wrap(~CASRN,labeller=labeller(CASRN=chem_labs),ncol=3)+
    scale_x_log10("Human Equivalent Dose POD (mg/kg-d)",
                  labels=scales::label_log())
print(ppods)
ggsave(file.path(resultsfolder,"Fig2-PODs.bychem.pdf"),ppods,height=10,width=6,scale=1.35)
ggsave(file.path(resultsfolder,"Fig2-PODs.bychem.svg"),ppods,height=10,width=6,scale=1.35)

## Most sensitive - median estimates
varnames <- c(pod.med_von.Borries="QSAR#3",
              pod.med_Kvasnicka="QSAR#2",
              pod.med_CTV="QSAR#1",
              pod.med_ToxCast="ToxCast",
              pod.med_Aurisano="ToxValDB",
              pod.med_PPRTV="qRAx")
min.pods <- data.frame(
...
pmin <-
  ...
  coord_flip()+scale_fill_viridis_d(direction = -1,end=0.9)+
  theme_bw()+theme(legend.position = "none")+
  ggtitle("Number (and %) of Minimum PODs")+
  xlab("")+ylab("")
print(pmin)
ggsave(file.path(resultsfolder,"Fig3B-mostsens.med.pdf"),pmin,height=4,width=6,scale=0.65)
ggsave(file.path(resultsfolder,"Fig3B-mostsens.med.svg"),pmin,height=4,width=6,scale=0.65)

## Lower bounds
varnames.lb <- c(pod.med.lb_von.Borries="QSAR#3",
              pod.med.lb_Kvasnicka="QSAR#2",
              pod.med.lb_CTV="QSAR#1",
              pod.med.lb_ToxCast="ToxCast",
              pod.med.lb_Aurisano="ToxValDB",
              pod.med.lb_PPRTV="qRAx")
...
print(pmin.lb)
ggsave(file.path(resultsfolder,"FigS3A-mostsens.lb.pdf"),pmin.lb,height=4,width=6,scale=0.65)
ggsave(file.path(resultsfolder,"FigS3A-mostsens.lb.svg"),pmin.lb,height=4,width=6,scale=0.65)

## Lower bound PPRTV with median for others
varnames.lb.med <- c(pod.med_von.Borries="QSAR#3",
                 pod.med_Kvasnicka="QSAR#2",
                 pod.med_CTV="QSAR#1",
                 pod.med_ToxCast="ToxCast",
                 pod.med_Aurisano="ToxValDB",
...
pmin.lb.med <-
  ...
  xlab("")+ylab("Frequency out of 41 (%)")
print(pmin.lb.med)
ggsave(file.path(resultsfolder,"FigS3B-mostsens.lb.med.pdf"),pmin.lb.med,height=4,width=6,scale=0.65)
ggsave(file.path(resultsfolder,"FigS3B-mostsens.lb.med.svg"),pmin.lb.med,height=4,width=6,scale=0.65)
...
pcibox<-
  ...
  ggtitle("POD Precision (Fold Uncertainty)")+
  scale_x_continuous(breaks=0:3,labels=c(1,10,100,1000))+
  xlab("")+
  annotation_logticks(side="b",outside=TRUE,
                      short=unit(0.02,"cm"),
  ...
  theme_bw()+theme(legend.position = "none")
print(pcibox)
ggsave(file.path(resultsfolder,"Fig3C-CI.pdf"),pcibox,height=4,width=6,scale=0.65)
ggsave(file.path(resultsfolder,"Fig3C-CI.svg"),pcibox,height=4,width=6,scale=0.65)
...
res.wide.mat$UF.bin <- ifelse(res.wide.mat$UF>30,"UF>30","UF≤30")
res.wide.mat$UF.bin <- factor(res.wide.mat$UF.bin,
                              levels=c("UF≤30","UF>30"))

lowerfun <- function(data,mapping) {
  ggplot(data = data, mapping = mapping)+
    geom_point()+annotation_logticks(linewidth=0.25)+
    scale_x_log10(limits=c(1e-5,1e3),labels=scales::label_log())+
    scale_y_log10(limits=c(1e-5,1e3),labels=scales::label_log())+
...
ggsave(file.path(resultsfolder,"Fig5-point-median.svg"),pscat.med,height=6,width=6,scale=1.4)
...
res.wide.lb.mat$UF.bin <- ifelse(res.wide.lb.mat$UF>30,"UF>30","UF≤30")
...
lowerfun.lb <- function(data,mapping) {
  ggplot(data = data, mapping = mapping)+
    geom_point()+annotation_logticks(linewidth=0.25)+
    scale_x_log10(limits=c(1e-6,1e2),labels=scales::label_log())+
    scale_y_log10(limits=c(1e-6,1e2),labels=scales::label_log())+
    geom_abline(slope=1,intercept = c(-1,0,1),color="grey",
                linetype=c("dotted","dashed","dotted"))
}
pscat.lb <-
...
ggsave(file.path(resultsfolder,"FigS4-lb-lb.svg"),pscat.lb,height=6,width=6,scale=1.4)
...
res.wide.lb.med.mat$UF.bin <- ifelse(res.wide.lb.med.mat$UF>30,"UF>30","UF≤30")
...
lowerfun.lb.med <- function(data,mapping) {
  ggplot(data = data, mapping = mapping)+
    geom_point()+annotation_logticks(linewidth=0.25)+
    scale_x_log10(limits=c(1e-6,1e3),labels=scales::label_log())+
    scale_y_log10(limits=c(1e-6,1e3),labels=scales::label_log())+
    geom_abline(slope=1,intercept = c(-1,0,1),color="grey",
                linetype=c("dotted","dashed","dotted"))
}
pscat.lb.med<-
...
ggsave(file.path(resultsfolder,"FigS5-lb-med.svg"),pscat.lb.med,height=6,width=6,scale=1.4)
...
ggsave(file.path(resultsfolder,"Fig4A-fold.svg"),pfold,height=4,width=6,scale=0.75)
ggsave(file.path(resultsfolder,"Fig4B-fold.lb.svg"),pfold.lb,height=4,width=6,scale=0.75)
...
ggsave(file.path(resultsfolder,"FigS1-pairs.svg"),psuccess.comb.2,height=4,width=6,scale=1.25)
...
ggsave(file.path(resultsfolder,"FigS2-three.svg"),psuccess.comb.3,height=6,width=6,scale=1.5)

## COMBINE FIGURE 3

fig3<-ggarrange(psuccess,pmin,pcibox,ncol=1,labels = "AUTO")
ggsave(file.path(resultsfolder,"Fig3_combined.pdf"),fig3,
       height=5.5,width=2.5,scale=1.6)

ggsave(file.path(resultsfolder,"Fig3_combined.svg"),fig3,
       height=5.5,width=2.5,scale=1.6)


## COMBINE FIGURE 4

fig4<-ggarrange(pfold,pfold.lb,ncol=1,labels = "AUTO")
ggsave(file.path(resultsfolder,"Fig4_combined.pdf"),fig4,
       height=4,width=2.5,scale=1.6)

ggsave(file.path(resultsfolder,"Fig4_combined.svg"),fig4,
       height=4,width=2.5,scale=1.6)


## COMBINE FIGURE S3

figS3<-ggarrange(pmin.lb,pmin.lb.med,ncol=1,labels = "AUTO")
ggsave(file.path(resultsfolder,"FigS3_combined.pdf"),figS3,
       height=4,width=2.5,scale=1.6)
ggsave(file.path(resultsfolder,"FigS3_combined.svg"),figS3,
       height=4,width=2.5,scale=1.6)
